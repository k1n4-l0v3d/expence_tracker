...
<div id="chat-input-area">
  ...
  <button id="chat-mic" title="Голосовой ввод"><i class="bi bi-mic-fill"></i></button>
  <button id="chat-send" title="Отправить"><i class="bi bi-send-fill"></i></button>
        ...
        if (!SR) {
            ...
            sr.onend = () => {
                ...
                if (sr._stream) {
                    sr._stream.getTracks().forEach(t => t.stop());
                    sr._stream = null;
                }
            };
            sr.onerror = () => {
            ...
            while (true) {
                ...
                for (const line of lines) {
                    ...
                    if (evt.done) {
                        ...
                        if (evt.url)        setTimeout(() => window.open(evt.url, '_self'), 300);
                        if (evt.open_help)  setTimeout(() => { if (typeof openHelp === 'function') openHelp(); }, 300);
                    }
                }
            }
        } catch {
